...
export function WirePanel({ onStatus }: { onStatus?: (s: WireStatus) => void }) {
  ...
  // The tab strip owns one TopBar for four panels, so the panel that is
  // actually on the wire is the one that reports the connection.
  ...
  if (!team) {
    return <div className="card"><div className="empty">You need a team before you can claim anybody.</div></div>;
  }
