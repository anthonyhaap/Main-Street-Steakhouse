...
/**
 * The league's transaction history — the account of everything the other three
 * tabs did.
 *
...
export function LedgerPanel({ onStatus }: { onStatus?: (s: WireStatus) => void }) {
  ...
  if (!ready || (!data && !error)) {
    return <div className="card"><SkeletonRows n={8} /></div>;
  }

  // Said out loud rather than left as a skeleton that never resolves — the same
  // guard the wire and the desk carry.
  if (error) {
    return (
      <div className="card">
