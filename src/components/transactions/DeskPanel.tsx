...
/** The trade desk: offers in, offers out, and who is on the block. */
export function DeskPanel({ onStatus }: { onStatus?: (s: WireStatus) => void }) {
  ...
  const { data, error, status, refetch } = useLive(fetcher, {
  ...
  // One TopBar serves all four tabs, so the panel on screen is the one that
  // reports whether the wire is live.
  ...
  const skeleton = <div className="card"><SkeletonRows n={8} /></div>;

  if (!ready) return skeleton;
  if (!team) {
    return <div className="card"><div className="empty">You need a team before you can trade.</div></div>;
  }
  // Said out loud rather than left as a skeleton that never resolves — the same
  // guard the wire carries.
  if (error) {
    return (
      <div className="card">
