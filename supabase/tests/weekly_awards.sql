...
begin
  ...
  -- They are ordinary 'event' lines in the League Feed, same as a trade or a
  -- waiver — this is the read the League/Chat split (20260919040000) gave
  -- activity_events, replacing the old merged ff_house_feed.
  if (select count(*) from jsonb_array_elements(
        (ff_league_feed(v_league, null, 100))->'items') x
       where x->>'source' = 'event' and x->>'kind' = 'award') <> 4 then
    raise exception 'the award cards did not surface in the League Feed';
  end if;
  v_checks := v_checks + 1;
