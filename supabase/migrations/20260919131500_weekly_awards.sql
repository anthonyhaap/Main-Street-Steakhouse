-- ============================================================================
-- Weekly awards: the Special's own facts, as cards in the House.
--
-- ff_week_recap already works out the high score, the low score, the widest
-- margin, the closest game, the bench mistake and the week's best player, every
-- week — but they only ever surface bundled into one prose message, which
-- reads once and then scrolls away like anything else said. The House already
-- renders activity_events as its own kind of line, with its own icon, sitting
-- next to the talk about it; a trade gets that treatment and the week's best
-- score does not, for no better reason than the recap having been built first.
--
-- So ff_publish_recap now also posts one activity_events row per award it has
-- a fact for, right alongside the prose recap it already writes. `award` joins
-- the event_type list the same way `transaction` did in 20260905022429: the
-- existing kinds are untouched. Idempotency rides on the recap row's own
-- lock — awards are posted only in the branch where the insert into
-- league_recaps actually happened, so a second call, like the message, is a
-- no-op rather than a second set of cards.
--
...
-- points" is worth finding in the House, not worth a phone going off about,
-- same call reactions.sql made about reactions themselves.
-- ============================================================================
