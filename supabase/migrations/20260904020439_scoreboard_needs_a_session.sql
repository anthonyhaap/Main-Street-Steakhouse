-- ============================================================================
-- The Sunday board needs a session.
--
-- `ff_scoreboard` shipped without a grant line, so it kept Postgres' default
-- of EXECUTE to PUBLIC — which on this project means `anon`. Every sibling
-- (`ff_briefing`, `ff_playoff_outlook`, `ff_team_hub`) is authenticated-only,
-- and for the reason `20260827025356` and `20260904…_second_sitting` both
-- write down: signup is open and the league id ships inside the client bundle,
-- so anything `anon` may call is effectively public. The board carries every
-- team, every lineup and every score in the week.
--
-- Two changes, and the second is the one that survives a mistake:
--
--   1. The grant is corrected to match its siblings.
--
--   2. The guard no longer waves through a caller with no `auth.uid()`. The
--      sibling functions read `if v_uid is not null and not a member` and lean
--      on their grants to keep the null case unreachable; this one says no
--      outright, so a future grant cannot re-open it. Nothing calls it without
--      a session — /matchups is behind the proxy and fetches in the browser.
--
-- Restated in full, as always. Nothing else about it changes.
--
-- Ledger refresh:
