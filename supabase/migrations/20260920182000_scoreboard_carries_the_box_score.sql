-- ============================================================================
-- The board carries the box score, not just the total.
--
-- `ff_scoreboard` already reads `player_stat_lines.stats` once, inside a
-- scalar subquery, to fold it through `ff_score` into a single number. The
-- browser only ever saw that number and a game clock — "9.4" and "Q2 8:41" —
-- which answers "how many points" but not "why": a QB's 9.4 could be one bad
-- pick or a whole quarter of nothing. The raw line is already sitting in the
-- same row; this hands the client the object itself alongside the point
