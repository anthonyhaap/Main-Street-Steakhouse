...
declare
  ...
  v_picks_total   integer;
  v_players       integer;
  v_with_adp      integer;
  v_with_bye      integer;
  v_games         integer;
  v_next_kick     timestamptz;
  v_week          integer;
  v_matchups      integer;
  v_rules         integer;
  v_last_stats    timestamptz;
  v_last_live     timestamptz;
  v_queue_teams   integer;
  v_open_chal     integer;
  v_msgs          integer;
  v_managers      jsonb;
  v_activity      jsonb;
...
begin
  ...
  select * into v_draft from drafts where league_id = p_league_id order by created_at limit 1;
  ...
  select count(distinct draft_slot) = v_teams
    into v_slots_ok
  ...
  select count(*),
         count(*) filter (where bye_week is not null)
    into v_players, v_with_bye
    from players where position in ('QB','RB','WR','TE','K','DST');

  select count(*) into v_with_adp
    from player_adp where season = v_league.season;
  ...
  select max(created_at) into v_last_stats from player_stat_lines;
  select max(created_at) into v_last_live
    from ingest_log where source in ('live','sleeper_stats','poll_live');

  select count(distinct team_id) into v_queue_teams
    from draft_queue q join teams t on t.id = q.team_id
   where t.league_id = p_league_id;

  select count(*) into v_open_chal
    from challenges
  ...
  from (
    select jsonb_build_object(
      'id', e.id, 'type', e.event_type, 'headline', e.headline,
  ...
  v_checks := jsonb_build_array(
    jsonb_build_object('key','commish','label','Commissioner claimed',
      ...
      'detail', case when v_league.commissioner_id is not null
                     then 'Locked to one account.'
                     else 'Nobody owns the controls yet — claim it before invites go out.' end,
      'fix','/admin'),
    jsonb_build_object('key','invited','label','All managers invited',
      'ok', v_invited >= v_teams and v_teams > 0,
      'detail', v_invited || ' of ' || v_teams || ' seats have an email on file.',
      'fix','/admin'),
    jsonb_build_object('key','joined','label','All managers signed in',
      'ok', v_joined >= v_teams and v_teams > 0,
      'detail', v_joined || ' of ' || v_teams || ' have created a login.',
      'fix','/admin'),
    jsonb_build_object('key','order','label','Draft order set',
      ...
      'detail', case when v_slots_ok then 'Slots 1-' || v_teams || ' assigned, no duplicates.'
                     else v_slots_set || ' of ' || v_teams || ' slots assigned. Randomize to finish.' end,
      'fix','/admin'),
    jsonb_build_object('key','pool','label','Player pool loaded',
      'ok', v_players > 500,
      'detail', v_players || ' skill players in the pool.',
      'fix','/players'),
    jsonb_build_object('key','adp','label','ADP loaded',
      'ok', v_with_adp > 100,
      'detail', v_with_adp || ' players ranked for ' || v_league.season || '.',
      'fix','/admin'),
    jsonb_build_object('key','byes','label','Bye weeks populated',
      'ok', v_with_bye > 500,
      'detail', v_with_bye || ' of ' || v_players || ' players have a bye on file.',
      'fix','/admin'),
    jsonb_build_object('key','schedule','label','NFL schedule loaded',
      'ok', v_games > 200,
      'detail', v_games || ' regular-season games for ' || v_league.season || '.',
      'fix','/admin'),
    jsonb_build_object('key','scoring','label','Scoring rules published',
      ...
      'detail', case when v_rules > 0 then v_rules || ' rule set(s) on record.'
                     else 'Running league defaults.' end,
      'fix','/admin'),
    jsonb_build_object('key','matchups','label','Season schedule generated',
      ...
      'detail', case when v_matchups > 0 then v_matchups || ' matchups scheduled.'
                     else 'Generates automatically when the draft completes.' end,
      'fix','/matchups'),
    jsonb_build_object('key','live','label','Live scoring wire',
      ...
      'detail', case when v_last_live is null then 'No ingest recorded yet.'
                     else 'Last ingest ' || to_char(v_last_live at time zone 'UTC','Mon DD HH24:MI') || ' UTC.' end,
      'fix','/admin')
  );

  select count(*) filter (where (c->>'ok')::boolean), count(*)
    into v_ready, v_total_checks
  ...
  return jsonb_build_object(
    ...
    'draft', case when v_draft.id is null then null else jsonb_build_object(
      ...
      'picks_made', v_picks_made, 'picks_total', v_picks_total,
      'order_set', v_slots_ok, 'teams_with_queue', v_queue_teams) end,
    'managers', v_managers,
    'checks', v_checks,
    ...
    'data', jsonb_build_object(
      'players', v_players, 'adp', v_with_adp, 'byes', v_with_bye,
      'games', v_games, 'last_stats_at', v_last_stats, 'last_ingest_at', v_last_live),
    'season', jsonb_build_object(
      'week', v_week, 'next_kickoff', v_next_kick, 'calendar', v_calendar),
    'clubhouse', jsonb_build_object(
      'open_challenges', v_open_chal, 'messages_7d', v_msgs),
    'activity', v_activity,
    'generated_at', now()
  );
end;
...
comment on function public.ff_league_pulse(uuid) is
  'Commissioner dashboard payload: readiness checklist, manager join status, data freshness, season calendar and activity in one round trip.';