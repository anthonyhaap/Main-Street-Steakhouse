-- ============================================================================
-- Tonight's Table, second sitting: what the first review found.
--
--   1. `ff_all_games` and `ff_all_sides` were executable by any signed-in
--      account. Signup is open and the league id ships in the bundle, so a
--      stranger could read every matchup through them despite RLS. They are
--      internal to ff_briefing and ff_history, which call them as their
--      definer; the grant is revoked and only the checked entry points remain.
--
--   2. The current season's playoff weeks were all labelled 'playoff', so the
--      plaque and the manager titles — which look for 'final' — could never
--      record this season's champion. The last playoff week is now the final,
--      the one before it the semifinal, the rest quarterfinals.
--
--   3. Tuesday's recap read `board`, which by Tuesday is the week ahead. The
--      `last` object now carries its own board and its own matchup id.
--
--   4. A streak's endpoints were min(season) with min(week) taken separately,
--      which invents a game when a run crosses seasons. They are now the first
--      and last game of the run.
--
-- ff_briefing and ff_history are restated in full, as always.
-- ============================================================================
...
revoke all on function public.ff_all_sides(uuid) from public, anon, authenticated;
revoke all on function public.ff_streak(text[]) from public, anon, authenticated;
...
begin
  ...
  from (
    -- Endpoints are whole games, not a min of one column and a min of
    -- another: a run from 2025 week 15 into 2026 week 2 starts at week 15.
    select jsonb_build_object('manager', manager, 'n', count(*),
             'from', jsonb_build_object('season', (array_agg(season order by season, week))[1],
                                        'week',   (array_agg(week   order by season, week))[1]),
             'to',   jsonb_build_object('season', (array_agg(season order by season desc, week desc))[1],
                                        'week',   (array_agg(week   order by season desc, week desc))[1])) as x
