-- ============================================================================
-- ff_team_hub, with projections.
--
-- The hub answered "what has he done" and "what is he doing right now" and had
-- nothing to say about the only question a Saturday lineup actually turns on:
-- what is he expected to do. `player_projections` now carries Sleeper's weekly
-- numbers in the same stat shape as the actuals, so the same `ff_score` call
-- prices them with this league's rules — a projection here is what it is worth
-- *to us*, not a stranger's PPR figure.
--
-- Everything else is unchanged from the version this replaces. The whole
-- function is restated rather than patched because a migration that reads as a
-- diff is a migration nobody can review a year from now.
-- ============================================================================
...
begin
  ...
  -- What he is expected to score this week, in our scoring.
  proj as (
    select pj.player_id,
           round(public.ff_score(pj.stats, v_rules), 2) as points,
           pj.updated_at
      from player_projections pj
      join mine m on m.player_id = pj.player_id
     where pj.season = v_season and pj.season_type = 2 and pj.week = v_week
       and pj.source = 'sleeper'
  )
  ...
  from (
    select jsonb_build_object(
      ...
      'projection', pr.points,
      'projected_at', pr.updated_at,
    ...
    left join proj     pr on pr.player_id = m.player_id
  ) s;
  ...
  select jsonb_build_object(
    ...
    'projected_starters', coalesce(round(sum((r->>'projection')::numeric) filter (where r->>'slot' <> 'BN'), 2), 0),
